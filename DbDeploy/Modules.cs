...
{
    class Modules : SortedList<string, Modules>
    {
        ...
        public void load(TransactionScript script, Configuration config, int check, string dial)
        {
            ...
            if (dial.ToUpper() == "DB2")
            {
                ...
                SPString = "SELECT MODULESCHEMA, MODULENAME, FROM SYSCAT.MODULES WHERE MODULEID=17 AND MODULECSCHEMA = '" + restrictions[1].ToUpper() + "' " + "  ORDER BY MODULENAME ";
            ...
            else
            {
                ...
                SPString += " WHERE ROUTINE_TYPE = 'MODULE' ORDER BY SPECIFIC_NAME";
                ...
                for (dataIndex = 0; dataIndex < data.Rows.Count; dataIndex++)
                {
                        ...
                        )
                    {
                        ...
                        log.log(Logger.LogLevel.progress, "Modules loaded: " + name);
                        this.Add(name, new Module(database, catalog, schema, name));
                    }
        ...
        public void list()
        {
            ...
            foreach (Module Module in this.Values)
            {
                text = Module.catalog +
                    ...
                    Module.schema +
                    " - " +
                    Module.name; //+ " - " + Module.type + " - " + Module.remarks;
